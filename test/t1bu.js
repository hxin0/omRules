...
describe('tier 1 default business unit rules', function () {
    ...
    before('read file first', async function () {
        ...
        if ((tExcel.length == 0) || (tExcel[0].shipper == undefined)) {
            console.log('Tier 1 ' + ruleNames.businessUnit + ' rule has no data - skipping')
            this.skip();
        }
    ...
    it('should add t1 business unit for the trading partner', () => {
        ...
        for (i = 0; i < tExcel.length; i++) {
            if ((tExcel[i].code != ruleCode) || (tExcel[i].scac != scacCode)) {
                // start a new rule
                console.log('Creating Tier 1 ' + ruleNames.businessUnit + ' rule.');
